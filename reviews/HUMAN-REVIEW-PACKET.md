...
| Later endurance visual evidence (H-012) | Repeat genuine combined tool/backend-resource capture for the fresh endurance run | Endurance runtime/PID does not exist yet | Requires its own real process and screenshot | Follow the later immediate pre-traffic gate instructions | PENDING HUMAN REVIEW |
| Endurance wrong-listener preflight | Use a fresh owned backend and provision only its disposable database | No k6 traffic; PID `52187` at `/Users/phamngocgiabao/eshop-sut/backend` received 20 synthetic WF-03 accounts because the AI listener check did not fail closed | The process and DB are outside the authorized disposable runtime; Codex must not kill/reset/clean them unilaterally | Stop PID 52187 through the app that owns it; decide whether Codex may remove/reset the 20 synthetic accounts or whether they should remain | HUMAN DECISION REQUIRED |

...
| Later Stress/Spike/endurance visual evidence (H-012) | Repeat genuine combined tool/backend-resource capture for each fresh run | Those runtimes/PIDs do not exist yet | Each run needs its own real process and screenshot | Follow the later immediate pre-traffic gate instructions | PENDING HUMAN REVIEW |

...
Status: **PHASE I DESIGN REVIEW — OFFICIAL EXECUTION NOT PERFORMED**

The table preserves the real AI proposal, runtime concern/evidence, human
decision, and resulting design. Pilot values validate the harness only; they
are not official Load/Stress/Spike results, thresholds, or capacity evidence.

| Area | Original AI Proposal | Pilot Evidence / Concern | Human Decision | Final Design |
|---|---|---|---|---|
| Load VU count | Five steady VUs; separate conservative two-VU Pilot | Two-VU Pilot exercised the shared implementation but did not validate five-VU performance | Approve five VUs as a bounded planning input | Five active VUs using accounts 01..05; not a capacity claim |
...
| Load duration | Five-minute hold then one-minute ramp-down | Pilot duration is not reusable as Load evidence | Approve planned Load duration and 30-second graceful settings | 5 VUs for 5m; 5→0 for 1m; 7m scheduled; 8m wall cap |
| Stress stage progression | Progressive 2→5→10→15→20, recovery to 5, then zero | No official Stress stage was exercised | Approve the bounded progressive schedule | 0→2/30s; alternating 1m holds/ramps at 2, 5, 10, 15, 20; 20→5/1m; hold 5/1m; 5→0/1m |
| Stress maximum | 20 VUs | Pilot validated only two VUs; SQLite and state growth remain risks | Approve 20 as an initial bounded maximum, never measured capacity | Maximum planned input 20 VUs; no automatic escalation |
| Stress stage duration | One-minute holds/ramps after a 30-second warm ramp | Short stages may be noisy and are not empirically validated | Approve exact 12m30s plan for first official Stress run | 12m30s scheduled; 30s graceful settings; 14m wall cap |
| Spike baseline | Three VUs with a two-minute observation after a 30-second ramp | Pilot did not exercise Spike shape | Approve three-VU baseline | 0→3/30s, then 3 VUs/2m |
| Spike peak | 20 VUs | Pilot validated only two VUs | Approve bounded 20-VU peak, not capacity | Peak target 20 VUs |
| Spike rise/fall | 3→20/10s and 20→3/10s | Abrupt transitions remain unexecuted | Approve genuine rapid rise/fall | Exact 10-second rise and 10-second fall |
| Spike duration | 20 VUs/45s, 3-VU recovery/2m, final 30s ramp-down | No official Spike evidence exists | Approve the planned observation windows | 6m5s scheduled; 30s graceful settings; 7m wall cap |
...
| Account mapping | Initially one seeded account; later dedicated account design | Pilot verified VU 1/2 dedicated identities; 20-account path is not yet executed | Up to 20 provisioned accounts; `exec.vu.idInTest` guard; no wrapping/sharing | Load uses 01..05; Stress/Spike require complete 01..20 pool |
...
| Test-data exhaustion | Public 20 rows plus private pool; fail on missing mapping | Pilot proved two-row binding; 20-account provisioning remains unexecuted | Incomplete required pool is preflight failure; no fallback or automatic load reduction | Exactly 20 valid credentials before any official run; only first five active in Load |
| Persistent order growth | Estimate one persistent canceled row per successful lifecycle | Pilot left canceled rows only in its disposable DB; 81 is not an official growth forecast | Fresh comparable disposable runtime for every scenario and preserve residual lifecycle evidence | Record start/end counts, never reuse scenario DB state, do not delete evidence mid-run |
| SQLite contention risk | Source-backed risk from login updates, order insert/update, delete journaling, and no configured busy timeout | Two-VU Pilot had no failed requests; this does not clear 20-VU contention risk | Observe under bounded official runs; do not call risk proven or absent beforehand | Monitor errors/latency/resources and retain real failure evidence; 20-VU cap remains |
| Report/output mapping | Load summary, Stress CSV, Spike dashboard; initial concern over literal JTL/HTML wording | Pinned k6 verified native JSON/CSV/dashboard capabilities; no official reports exist | Use PDF-permitted k6 equivalents; never fabricate or rename JTL | All preserve native JSON; genuinely distinct Load aggregate, Stress CSV analysis, Spike dashboard; real per-run report dirs |
...
The shared WF-03 implementation is runtime-validated by the corrected 2-VU
Pilot. The official workload inputs remain human-approved plans. No official
scenario, renderer output, final threshold, capacity finding, or attributable
test-plan filename exists yet.
...
| Output separation | All scenarios plan native JSON; only Stress adds CSV and only Spike adds dashboard HTML; Pilot remains under `results/pilot/` |
| Official runner | POSIX shell syntax and rejection guards passed; it is prepared/unexecuted and contains no business API logic |
